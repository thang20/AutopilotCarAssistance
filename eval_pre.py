    ...
    for step, (imgs, img_ids) in enumerate(val_loader):
        ...
        with torch.no_grad(): # (corresponds to setting volatile=True in all variables, this is done during inference to reduce memory consumption)
            ...
            for i in range(pred_label_imgs.shape[0]):
                ...
                img_id = img_ids[i]
                print('1')
                a = network.model_dir + "/videopre/" + img_id + "_pred_label_img.png"
                if not os.path.exists(a):
                    ...
                    pred_label_img = label_img_to_color(pred_label_img)
                    pred_label_img = cv2.resize(pred_label_img, (1024, 512))
                    pred_label_img = pred_label_img.astype(np.uint8)

                    #cv2.imshow("win", pred_label_img)

                    cv2.imwrite(network.model_dir + "/videopre/" + img_id + "_pred_label_img.png", pred_label_img)
                    #cv2.waitKey(1)
